// Cursor personalizado corazón
document.addEventListener('DOMContentLoaded', function() {
  const cursor = document.querySelector('.custom-cursor-heart');
  if (!cursor) return;
  ...
  // Elementos interactivos para hover
  const hoverElements = document.querySelectorAll('a, button, .noise_btn');
...
class NoiseButton extends PIXI.Application {
  ...
  async createCanvas() {
    ...
    // PIXI.Sprite.fromImage está obsoleto, usar PIXI.Sprite.from
    this.noiseSprite = PIXI.Sprite.from(this.options.displacementMap);
